...
void Updates::handleProcessFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    } else if (QByteArray output = this->process->readAllStandardOutput();
               output.isNull() || output.trimmed().isEmpty() || (output.indexOf(QByteArrayView("There are currently no updates available")) >= 0)) {
